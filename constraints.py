...
class Constraints():
    ...
    def hard_constraint_cost(self, constraints='all'):
        ...
        all_constraints = [self.hc1_primary_pto_protected,
    ...
    def soft_constraint_cost(self, constraints='all'):
        """select and apply hard constraints then sum penalty scores"""
        all_constraints = [self.sc1_no_double_back,
    ...
    def total_cost(self, hard_constraints, soft_constraints): # expects constraints e.g. [[1, 2], [2,3]]
        """Calculate penalty cost of a schedule"""
        # Set weights, select constraints and calculate hard and soft costs
        hard_weight = 1
        hard_cost = self.hard_constraint_cost(hard_constraints)[0] # (total, {constraint:total})[0]

        soft_weight = 1
        soft_cost = self.soft_constraint_cost(soft_constraints)[0] # (total, {constraint:total})[0]
